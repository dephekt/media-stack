...
- Single-value `.env` files get exposed as Docker Compose secrets and read at runtime via either the `secrets2env.sh` shim baked into our custom `ghcr.io/dephekt/*` images or a service-specific wrapper. Newt now uses the official `fosrl/newt` image with `core/newt-entrypoint.sh` exporting `NEWT_ID` and `NEWT_SECRET` from Docker secrets before exec'ing upstream.
